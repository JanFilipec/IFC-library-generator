...
# beds
def create_fancy_bed_representations(builder, self, overall_width, overall_depth, overall_height, blanket_tan=tan(pi / 8)):
...
# wardrobes
def create_fancy_wardrobe_representations(builder, self, overall_width, overall_depth, overall_height):
    curve = builder.rectangle(size=V(overall_width, overall_depth))
    mesh = builder.extrude(builder.profile(curve), overall_height)
    representation_3d = builder.get_representation(context=self.representations["model_body"], items=mesh)
...
# table
def create_fancy_rectangle_table(builder, self, width, depth, height, return_representations=False):
    output = []
    leg_size = 50.0
